...
class Turbine(object):
    ...
    def state(self, nodes):
        if self.ef==1.0:
           ...
           nodes[self.outNode].h=nodes[self.inNode].h-self.ef*(nodes[self.inNode].h-isoh)
        ...
        if self.extNode!=None:
           if self.ef==1.0:
              ...
              isoh=seuif97.ps2h(nodes[self.extNode].p,nodes[self.inNode].s)
              nodes[self.extNode].h=nodes[self.inNode].h-self.ef*(nodes[self.inNode].h-isoh)
              nodes[self.extNode].ph()
              
              isoh=seuif97.ps2h(nodes[self.outNode].p,nodes[self.extNode].s)
              nodes[self.outNode].h=nodes[self.extNode].h-self.ef*(nodes[self.extNode].h-isoh)
              nodes[self.outNode].ph()
    ...
    def fdot(self, nodes):
        if (self.fdotok==False):
           try:
                if  self.extNode!=None:
                    nodes[self.outNode].fdot=nodes[self.inNode].fdot-nodes[self.extNode].fdot
                else:    
                ...
                self.fdotok= self.fdotok and (nodes[self.extNode].fdot !=None)
    ...
    def simulate(self, nodes):
        ...
        if  self.extNode!=None:
             self.workExtracted = nodes[self.extNode].fdot * (nodes[self.inNode].h - nodes[self.extNode].h) 
        self.workExtracted  +=  nodes[self.outNode].fdot *( nodes[self.inNode].h -  nodes[self.outNode].h)    
    ...
    def sm_energy(self, nodes):
        ...
        if  self.extNode!=None:
            self.WExtracted = nodes[self.extNode].mdot * (nodes[self.inNode].h - nodes[self.extNode].h) 
         
        self.WExtracted  +=  nodes[self.outNode].mdot *( nodes[self.inNode].h -  nodes[self.outNode].h)  
